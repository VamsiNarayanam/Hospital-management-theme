(function () {
  ...
  function initFlightDeckChrome() {
    ...
    if (headerActions && !headerActions.querySelector(".flight-status")) {
      ...
      status.innerHTML = '<span class="flight-status__dot"></span>CARE ONLINE';
  ...
  function initHeroTelemetry() {
    ...
    function tick() {
      if (hr) hr.textContent = String(68 + Math.floor(Math.random() * 12));
      if (spo2) spo2.textContent = String(96 + Math.floor(Math.random() * 3));
      if (bp) bp.textContent = String(118 + Math.floor(Math.random() * 8)) + "/" + String(72 + Math.floor(Math.random() * 6));
      if (temp) temp.textContent = (97.8 + Math.random() * 0.8).toFixed(1);
